...
{
    ThreadTriTree T=new TriNode,which=NULL;
    BiTree S=bitree;
    T->data=S->data; 
    ...
    {   
        ...
        if (which==p->lchild)S=S->lchild;
        if (which==p->rchild)S=S->rchild;

        if (S->lchild != NULL)
        {
            p->lchild=new TriNode;
            p->lchild->data=S->lchild->data;
        }else p->lchild = NULL;

        if (p->rchild != NULL)
        {
            p->rchild=new TriNode;
            p->rchild->data=S->rchild->data;
        }else p->rchild = NULL; 
